...
use crate::api;
use crate::api::query;
use crate::api::query::{QueryAsync, RawQueryAsync};
            ...
            if let (Some(available_auth), false) = (self.state.get_any_valid_auth(), force_new_auth)
            {
                ...
                match auth_type {
                    ...
                    AuthType::V3Federation => {
                        ...
                        let init_auth_ep = auth::v3federation::get_auth_ep(&self.config, callback_addr.port())?;
                        ...
                        oidc_callback_builder.code(oauth2_code.code.clone());
                        oidc_callback_builder.state(oauth2_code.state.clone());
                        let oidc_callback_ep = oidc_callback_builder
                            .build()
                            .map_err(auth::v3federation::FederationError::from)?;

                        let mut token_auth: authtoken::AuthToken =
                            oidc_callback_ep.query_async(self).await?;

...
//! object with data content and metadata. The PUT operation always creates an
//! object. If you use this operation on an existing object, you replace the
//! existing object and metadata rather than modifying the object.
//! Consequently, this operation returns the Created (201) response code. If
//! you use this operation to copy a manifest object, the new object is a
...
//! than 5 GB. Note that the provider may have limited the characters which are
//! allowed in an object name. Any name limits are exposed under the name_check
//! key in the /info discoverability response. Regardless of name_check
//! limitations, names must be URL quoted UTF-8. To create custom metadata, use
//! the X-Object-Meta-name header, where name is the name of the metadata item.
//!

...
//! Permanently deletes an object from the object store. Object deletion occurs
//! immediately at request time. Any subsequent GET, HEAD, POST, or DELETE
//! operations will return a 404 Not Found error code. For static large object
//! manifests, you can add the ?multipart- manifest=delete query parameter.
//! This operation deletes the segment objects and, if all deletions succeed,
//! this operation deletes the manifest object. A DELETE request made to a
//! symlink path will delete the symlink rather than the target object. An
//! alternative to using the DELETE operation is to use the POST operation with
//! the bulk-delete query parameter.
//!

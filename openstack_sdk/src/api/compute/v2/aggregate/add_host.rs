...
/// The add\_host object used to add host to aggregate.
#[derive(Builder, Debug, Deserialize, Clone, Serialize)]
#[builder(setter(strip_option))]
pub struct AddHost<'a> {
...
impl<'a> RestEndpoint for Request<'a> {
    ...
    fn body(&self) -> Result<Option<(&'static str, Vec<u8>)>, BodyError> {
        ...
        params.push("add_host", serde_json::to_value(&self.add_host)?);
...
mod tests {
    ...
    fn test_service_type() {
        assert_eq!(
            Request::builder()
                .add_host(AddHostBuilder::default().host("foo").build().unwrap())
                .build()
                .unwrap()
                .service_type(),
            ServiceType::Compute

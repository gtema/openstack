...
use crate::api::compute::v2::server::migration::{get as Get, list as List};

/// Find for server/migration by nameOrId.
...
impl<'a> Findable for Request<'a> {
    ...
    /// Locate server/migration in a list
    fn locate_resource_in_list<C: RestClient>(
        &self,
        data: Vec<serde_json::Value>,
    ) -> Result<serde_json::Value, ApiError<C::Error>> {
        // server/migration is not supporting name as query parameter to the list.

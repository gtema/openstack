...
use crate::api::compute::v2::server::volume_attachment::{get as Get, list as List};

/// Find for server/volume_attachment by nameOrId.
...
impl<'a> Findable for Request<'a> {
    ...
    fn get_ep(&self) -> Get::Request<'a> {
        ...
        ep.id(self.id.clone());
        ep.server_id(self.server_id.clone());
    ...
    /// Locate server/volume_attachment in a list
    fn locate_resource_in_list<C: RestClient>(
        &self,
        data: Vec<serde_json::Value>,
    ) -> Result<serde_json::Value, ApiError<C::Error>> {
        // server/volume_attachment is not supporting name as query parameter to the list.

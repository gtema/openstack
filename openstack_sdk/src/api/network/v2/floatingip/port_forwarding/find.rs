...
/// Find for floatingip/port_forwarding by nameOrId.
...
impl<'a> Findable for Request<'a> {
    ...
    fn get_ep(&self) -> Get::Request<'a> {
        ...
        ep.id(self.id.clone());
        ep.floatingip_id(self.floatingip_id.clone());
    ...
    /// Locate floatingip/port_forwarding in a list
    fn locate_resource_in_list<C: RestClient>(
        &self,
        data: Vec<serde_json::Value>,
    ) -> Result<serde_json::Value, ApiError<C::Error>> {
        // floatingip/port_forwarding is not supporting name as query parameter to the list.

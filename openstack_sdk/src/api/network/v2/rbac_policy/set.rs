...
impl RestEndpoint for Request<'_> {
    ...
    fn endpoint(&self) -> Cow<'static, str> {
        format!("rbac-policies/{id}", id = self.id.as_ref(),).into()
    }
    ...
    fn body(&self) -> Result<Option<(&'static str, Vec<u8>)>, BodyError> {
        ...
        params.push("rbac_policy", serde_json::to_value(&self.rbac_policy)?);
...
mod tests {
    ...
    fn test_service_type() {
        assert_eq!(
            Request::builder()
                .rbac_policy(RbacPolicyBuilder::default().build().unwrap())
                .build()
                .unwrap()
                .service_type(),
            ServiceType::Network
    ...
    fn endpoint() {
        ...
        let mock = server.mock(|when, then| {
            when.method(httpmock::Method::PUT)
                .path(format!("/rbac-policies/{id}", id = "id",));

            then.status(200)
                .header("content-type", "application/json")
                .json_body(json!({ "rbac_policy": {} }));
        });

...
use crate::api::network::v2::router::conntrack_helper::{get as Get, list as List};

/// Find for router/conntrack_helper by nameOrId.
...
impl<'a> Findable for Request<'a> {
    ...
    fn get_ep(&self) -> Get::Request<'a> {
        ...
        ep.id(self.id.clone());
        ep.router_id(self.router_id.clone());
    ...
    /// Locate router/conntrack_helper in a list
    fn locate_resource_in_list<C: RestClient>(
        &self,
        data: Vec<serde_json::Value>,
    ) -> Result<serde_json::Value, ApiError<C::Error>> {
        // router/conntrack_helper is not supporting name as query parameter to the list.

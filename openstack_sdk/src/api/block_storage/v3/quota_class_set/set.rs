...
pub struct Request<'a> {
    #[builder(private, setter(into, name = "_quota_class_set"))]
...
impl<'a> RequestBuilder<'a> {
    pub fn quota_class_set<I, K, V>(&mut self, iter: I) -> &mut Self
    where
        I: Iterator<Item = (K, V)>,
        K: Into<Cow<'a, str>>,
        V: Into<i32>,
...
impl RestEndpoint for Request<'_> {
    ...
    fn endpoint(&self) -> Cow<'static, str> {
        format!("os-quota-class-sets/{name}", name = self.name.as_ref(),).into()
    }
    ...
    fn endpoint() {
        ...
        let mock = server.mock(|when, then| {
            when.method(httpmock::Method::PUT)
                .path(format!("/os-quota-class-sets/{name}", name = "name",));

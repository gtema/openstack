...
pub enum RoleCommands {
    Assignment(role_assignment::RoleAssignmentCommand),
    Create(create::RoleCommand),
    Delete(delete::RoleCommand),
    Imply(imply::ImplyCommand),
    Inference(role_inference::RoleInferenceCommand),
...
impl RoleCommand {
    ...
    pub async fn take_action<C: CliArgs>(
        ...
        match &self.command {
            RoleCommands::Assignment(cmd) => cmd.take_action(parsed_args, session).await,
            RoleCommands::Create(cmd) => cmd.take_action(parsed_args, session).await,
            RoleCommands::Delete(cmd) => cmd.take_action(parsed_args, session).await,
            RoleCommands::Imply(cmd) => cmd.take_action(parsed_args, session).await,
            RoleCommands::Inference(cmd) => cmd.take_action(parsed_args, session).await,

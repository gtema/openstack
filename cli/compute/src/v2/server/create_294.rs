...
//! Create Server command [microversion = 2.94]
//!
    ...
    pub async fn take_action<C: CliArgs>(
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_294::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| {
                    serde_json::from_value::<create_294::BlockDeviceMapping>(v.to_owned())
                })
        ...
        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_294::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| {
                    serde_json::from_value::<create_294::BlockDeviceMappingV2>(v.to_owned())
                })
        ...
        if args.networks.auto_networks {
            server_builder.networks(create_294::ServerNetworks::F2(
        ...
        if let Some(data) = &args.networks.networks {
            let networks_builder: Vec<create_294::Networks> = data
                .iter()
                .flat_map(|v| serde_json::from_value::<create_294::Networks>(v.to_owned()))
                .collect();
            server_builder.networks(create_294::ServerNetworks::F1(networks_builder));
        }

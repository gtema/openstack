...
//! Create Server command [microversion = 2.37]
//!
...
use openstack_sdk::api::compute::v2::server::get_20 as get;
    ...
    pub async fn take_action<C: CliArgs>(
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_237::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| {
                    serde_json::from_value::<create_237::BlockDeviceMapping>(v.to_owned())
                })
        ...
        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_237::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| {
                    serde_json::from_value::<create_237::BlockDeviceMappingV2>(v.to_owned())
                })
        ...
        if args.networks.auto_networks {
            server_builder.networks(create_237::ServerNetworks::F2(
        ...
        if let Some(data) = &args.networks.networks {
            let networks_builder: Vec<create_237::Networks> = data
                .iter()
                .flat_map(|v| serde_json::from_value::<create_237::Networks>(v.to_owned()))
                .collect();
            server_builder.networks(create_237::ServerNetworks::F1(networks_builder));
        }
        ...
        if let Some(val) = &args.personality {
            let personality_builder: Vec<create_237::Personality> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_237::Personality>(v.to_owned()))
        ...
        if self.wait {
            let wait_id = data
                ...
                .ok_or_eyre("create response is missing /server/id, cannot wait")?
                .to_string();
            ...
            op.output_single::<response::create_a::ServerResponse>(data.clone())
                .or_else(|_| {
                    op.output_single::<response::create_b::ServerResponse>(data.clone())
                })?;
        } else {
            op.output_single::<response::create_a::ServerResponse>(data.clone())
                .or_else(|_| {

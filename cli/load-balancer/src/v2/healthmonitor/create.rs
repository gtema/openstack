...
use openstack_sdk::api::load_balancer::v2::healthmonitor::get;
...
impl HealthmonitorCommand {
    ...
    pub async fn take_action<C: CliArgs>(
        ...
        if self.wait {
            let wait_id = data
                .pointer("/healthmonitor/id")
                .and_then(serde_json::Value::as_str)
                .ok_or_eyre("create response is missing /healthmonitor/id, cannot wait")?
                .to_string();
            ...
            op.output_single::<response::create::HealthmonitorResponse>(data.clone())?;
        } else {
            op.output_single::<response::create::HealthmonitorResponse>(data.clone())?;
        }

...
//! Create Allocation command [microversion = 1.13]
//!
...
use openstack_sdk::api::placement::v1::allocation::create_113;
...
#[derive(Args)]
#[command(about = "Manage allocations (microversion = 1.13)")]
...
impl AllocationCommand {
    ...
    pub async fn take_action(
        ...
        let op =
            OutputProcessor::from_args(parsed_args, Some("placement.allocation"), Some("create"));
        op.validate_args(parsed_args)?;

        let mut ep_builder = create_113::Request::builder();
        ep_builder.header("OpenStack-API-Version", "placement 1.13");
        ...
        if let Some(properties) = &self.properties {
            ep_builder.properties(
                properties
                    .iter()
                    .map(|(k, v)| {
                        serde_json::from_value(v.to_owned()).map(|v: create_113::Item| (k, v))
                    })

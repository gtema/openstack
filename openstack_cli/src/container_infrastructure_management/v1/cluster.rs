...
pub enum ClusterCommands {
    Create(Box<create::ClusterCommand>),
    Delete(Box<delete::ClusterCommand>),
    List(Box<list::ClustersCommand>),
    Nodegroup(Box<nodegroup::NodegroupCommand>),
...
impl ClusterCommand {
    ...
    pub async fn take_action(
        ...
        match &self.command {
            ClusterCommands::Create(cmd) => cmd.take_action(parsed_args, session).await,
            ClusterCommands::Delete(cmd) => cmd.take_action(parsed_args, session).await,
            ClusterCommands::List(cmd) => cmd.take_action(parsed_args, session).await,
            ClusterCommands::Nodegroup(cmd) => cmd.take_action(parsed_args, session).await,

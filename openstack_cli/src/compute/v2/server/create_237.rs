...
use clap::ValueEnum;
use openstack_sdk::api::compute::v2::server::create_237;
...
#[command(about = "Create Server (microversion = 2.37)")]
...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        info!("Create Server with {:?}", self.args);

        let op = OutputProcessor::from_args(parsed_args);
        op.validate_args(parsed_args)?;
        info!("Parsed args: {:?}", self.args);

        let mut ep_builder = create_237::Request::builder();
        ep_builder.header("OpenStack-API-Version", "compute 2.37");
        ...
        let mut server_builder = create_237::ServerBuilder::default();
        ...
        if let Some(data) = &args.networks.networks {
            let networks_builder: Vec<create_237::Networks> = data
                .iter()
                .flat_map(|v| serde_json::from_value::<create_237::Networks>(v.clone()))
                .collect();
        ...
        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_237::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_237::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.personality {
            let personality_builder: Vec<create_237::Personality> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_237::Personality>(v.clone()))
                .collect::<Vec<create_237::Personality>>();
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_237::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_237::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_237::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_237::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_237::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_237::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_237::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_237::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_237::OsSchedulerHintsBuilder::default();

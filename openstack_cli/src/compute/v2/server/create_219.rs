...
use clap::ValueEnum;
use openstack_sdk::api::compute::v2::server::create_219;
...
#[command(about = "Create Server (microversion = 2.19)")]
...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        let mut ep_builder = create_219::Request::builder();
        ...
        let mut server_builder = create_219::ServerBuilder::default();
        ...
        if let Some(val) = &args.networks {
            let networks_builder: Vec<create_219::Networks> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_219::Networks>(v.clone()))
                .collect::<Vec<create_219::Networks>>();
            server_builder.networks(networks_builder);
        }

        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_219::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_219::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.personality {
            let personality_builder: Vec<create_219::Personality> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_219::Personality>(v.clone()))
                .collect::<Vec<create_219::Personality>>();
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_219::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_219::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_219::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_219::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_219::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_219::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_219::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_219::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_219::OsSchedulerHintsBuilder::default();

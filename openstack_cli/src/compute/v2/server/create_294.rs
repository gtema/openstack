...
use clap::ValueEnum;
use openstack_sdk::api::compute::v2::server::create_294;
...
#[command(about = "Create Server (microversion = 2.94)")]
...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        let mut ep_builder = create_294::Request::builder();
        ...
        let mut server_builder = create_294::ServerBuilder::default();
        ...
        if let Some(data) = &args.networks.networks {
            let networks_builder: Vec<create_294::Networks> = data
                .iter()
                .flat_map(|v| serde_json::from_value::<create_294::Networks>(v.clone()))
                .collect();
        ...
        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_294::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_294::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_294::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_294::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_294::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_294::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_294::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_294::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_294::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_294::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(val) = &args.hostname {
            server_builder.hostname(val.clone());
        }
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_294::OsSchedulerHintsBuilder::default();

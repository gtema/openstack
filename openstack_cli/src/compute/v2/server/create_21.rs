...
use clap::ValueEnum;
use openstack_sdk::api::compute::v2::server::create_21;
...
#[command(about = "Create Server (microversion = 2.1)")]
...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        let mut server_builder = create_21::ServerBuilder::default();
        ...
        if let Some(val) = &args.networks {
            let networks_builder: Vec<create_21::Networks> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_21::Networks>(v.clone()))
                .collect::<Vec<create_21::Networks>>();
            server_builder.networks(networks_builder);
        }

        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_21::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_21::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.personality {
            let personality_builder: Vec<create_21::Personality> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_21::Personality>(v.clone()))
                .collect::<Vec<create_21::Personality>>();
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_21::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_21::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_21::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_21::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_21::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_21::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_21::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_21::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_21::OsSchedulerHintsBuilder::default();

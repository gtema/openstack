...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        let mut ep_builder = create_242::Request::builder();
        ep_builder.header("OpenStack-API-Version", "compute 2.42");
        ...
        let mut server_builder = create_242::ServerBuilder::default();
        ...
        if let Some(data) = &args.networks.networks {
            let networks_builder: Vec<create_242::Networks> = data
                .iter()
                .flat_map(|v| serde_json::from_value::<create_242::Networks>(v.clone()))
                .collect();
        ...
        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_242::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_242::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.personality {
            let personality_builder: Vec<create_242::Personality> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_242::Personality>(v.clone()))
                .collect::<Vec<create_242::Personality>>();
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_242::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_242::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_242::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_242::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_242::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_242::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_242::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_242::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_242::OsSchedulerHintsBuilder::default();

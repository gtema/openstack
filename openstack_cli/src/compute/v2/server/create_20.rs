...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_20::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_20::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_20::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_20::SecurityGroupsBuilder::default().name(v).build())
                .collect();

...
#[command(about = "Create Server (microversion = 2.33)")]
...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        let mut ep_builder = create_233::Request::builder();
        ep_builder.header("OpenStack-API-Version", "compute 2.33");
        ...
        let mut server_builder = create_233::ServerBuilder::default();
        ...
        if let Some(val) = &args.networks {
            let networks_builder: Vec<create_233::Networks> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_233::Networks>(v.clone()))
                .collect::<Vec<create_233::Networks>>();
            server_builder.networks(networks_builder);
        }

        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_233::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_233::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.personality {
            let personality_builder: Vec<create_233::Personality> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_233::Personality>(v.clone()))
                .collect::<Vec<create_233::Personality>>();
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_233::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_233::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_233::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_233::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_233::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_233::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_233::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_233::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_233::OsSchedulerHintsBuilder::default();

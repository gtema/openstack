...
use clap::ValueEnum;
use openstack_sdk::api::compute::v2::server::create_263;
...
#[command(about = "Create Server (microversion = 2.63)")]
...
impl OSCCommand for ServerCmd {
    async fn take_action(
        ...
        let mut ep_builder = create_263::Request::builder();
        ...
        let mut server_builder = create_263::ServerBuilder::default();
        ...
        if let Some(data) = &args.networks.networks {
            let networks_builder: Vec<create_263::Networks> = data
                .iter()
                .flat_map(|v| serde_json::from_value::<create_263::Networks>(v.clone()))
                .collect();
        ...
        if let Some(val) = &args.os_dcf_disk_config {
            let tmp = match val {
                OsDcfDiskConfig::Auto => create_263::OsDcfDiskConfig::Auto,
                OsDcfDiskConfig::Manual => create_263::OsDcfDiskConfig::Manual,
            };
        ...
        if let Some(val) = &args.block_device_mapping {
            let block_device_mapping_builder: Vec<create_263::BlockDeviceMapping> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_263::BlockDeviceMapping>(v.clone()))
                .collect::<Vec<create_263::BlockDeviceMapping>>();
            server_builder.block_device_mapping(block_device_mapping_builder);
        }

        if let Some(val) = &args.block_device_mapping_v2 {
            let block_device_mapping_v2_builder: Vec<create_263::BlockDeviceMappingV2> = val
                .iter()
                .flat_map(|v| serde_json::from_value::<create_263::BlockDeviceMappingV2>(v.clone()))
                .collect::<Vec<create_263::BlockDeviceMappingV2>>();
        ...
        if let Some(val) = &args.security_groups {
            let security_groups_builder: Vec<create_263::SecurityGroups> = val
                .iter()
                .flat_map(|v| create_263::SecurityGroupsBuilder::default().name(v).build())
                .collect();
        ...
        if let Some(args) = &self.args.os_scheduler_hints {
            let mut os_scheduler_hints_builder = create_263::OsSchedulerHintsBuilder::default();

...
//!
//! Wraps invoking of the `v2.0/routers/{id}/update_external_gateways` with `PUT` method
...
/// Request body
#[derive(Args)]
#[command(about = "Update external gateways of router")]
...
impl RouterCommand {
    ...
    pub async fn take_action(
        ...
        let mut router_builder = update_external_gateways::RouterBuilder::default();
        if let Some(val) = &args.external_gateways {
            let external_gateways_builder: Vec<update_external_gateways::ExternalGateways> = val
                .iter()
                .flat_map(|v| {
                    serde_json::from_value::<update_external_gateways::ExternalGateways>(
                        v.to_owned(),
                    )
                })
                .collect::<Vec<update_external_gateways::ExternalGateways>>();

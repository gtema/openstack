...
//! List LocalIps command
//!
//! Wraps invoking of the `v2.0/local-ips` with `GET` method
...
struct QueryParameters {
    ...
    /// ip_mode query parameter for /v2.0/local-ips API
...
impl LocalIpsCommand {
    ...
    pub async fn take_action(
        ...
        let op = OutputProcessor::from_args(parsed_args, Some("network.local_ip"), Some("list"));
        ...
        if let Some(val) = &self.query.ip_mode {
            ep_builder.ip_mode(val);
        }
        if let Some(val) = &self.query.local_ip_address {
            ep_builder.local_ip_address(val);
        }
        if let Some(val) = &self.query.local_port_id {
            ep_builder.local_port_id(val);
        }

...
pub struct Server<'a> {
    ...
    /// This is the availability zone the server is pinned to, either at
    /// creation or via update request. To unpin, set to `null`. The server
    /// will then be free to move to any availability zone on the next move
    /// operation. To re-pin, set to the server’s current
    /// `OS-EXT-AZ:availability_zone` value. A server can only be pinned to the
    /// zone it currently resides in. Setting to a different zone or changing
    /// directly from one non-null value to another will result in a 409 error.
    ///
    /// **New in version 2.104**
    #[serde(skip_serializing_if = "Option::is_none")]
    #[builder(default, setter(into))]
    pub(crate) pinned_availability_zone: Option<Option<Cow<'a, str>>>,
}

...
impl<'a> RequestBuilder<'a> {
    /// Add a single header to the Instance_Usage_Audit_Log.
...
impl RestEndpoint for Request<'_> {
    ...
    fn endpoint(&self) -> Cow<'static, str> {
        format!("os-instance_usage_audit_log/{id}", id = self.id.as_ref(),).into()
    }
    ...
    fn response_key(&self) -> Option<Cow<'static, str>> {
        Some("instance_usage_audit_log".into())
    }
    ...
    fn endpoint() {
        ...
        let mock = server.mock(|when, then| {
            when.method(httpmock::Method::GET)
                .path(format!("/os-instance_usage_audit_log/{id}", id = "id",));

            then.status(200)
                .header("content-type", "application/json")
                .json_body(json!({ "instance_usage_audit_log": {} }));
        });

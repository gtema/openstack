...
pub static PLUGIN: TokenAuthenticator = TokenAuthenticator;
inventory::submit! {
...
impl TokenAuthenticator {
    ...
    ) -> Result<(&'static str, Value), TokenAuthError> {
        let token = values
            .get("token")
            .ok_or(TokenAuthError::MissingToken)?
            .expose_secret();
        let body = json!({ "token": { "id": token } });
        Ok(("token", body))
    }
}

impl OpenStackMultifactorAuthMethod for TokenAuthenticator {
...
#[async_trait]
impl OpenStackAuthType for TokenAuthenticator {
    fn get_supported_auth_methods(&self) -> Vec<&'static str> {
        self._get_supported_auth_methods()
    }
...
impl From<TokenAuthError> for AuthError {
    fn from(source: TokenAuthError) -> Self {
        Self::plugin(source)
    }
}

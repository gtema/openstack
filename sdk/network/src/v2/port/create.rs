...
pub const BODY_SCHEMA: &str = "{\"description\": \"Request of the ports:post operation\", \"properties\": {\"port\": {\"description\": \"A `port` object.\", \"properties\": {\"admin_state_up\": {\"description\": \"The administrative state of the resource, which is\\nup (`true`) or down (`false`).\\nDefault is `true`.\", \"type\": [\"boolean\", \"string\"]}, \"allowed_address_pairs\": {\"description\": \"A set of zero or more allowed address pair objects each where address pair\\nobject contains an `ip_address` and `mac_address`. While the\\n`ip_address` is required, the `mac_address` will be taken from the\\nport if not specified. The value of `ip_address` can be an IP Address\\nor a CIDR (if supported by the underlying extension plugin).\\nA server connected to the port can send a packet with source address which\\nmatches one of the specified allowed address pairs.\", \"items\": {\"properties\": {\"ip_address\": {\"type\": \"string\"}, \"max_address\": {\"type\": \"string\"}}, \"type\": \"object\"}, \"type\": \"array\"}, \"binding:host_id\": {\"description\": \"The ID of the host where the port resides.\\nThe default is an empty string.\", \"type\": \"string\"}, \"binding:profile\": {\"description\": \"A dictionary that enables the application running on the specific host to\\npass and receive vif port information specific to the networking back-end.\\nThis field is only meant for machine-machine communication for compute\\nservices like Nova, Ironic or Zun to pass information to a Neutron\\nback-end. It should not be used by multiple services concurrently or by\\ncloud end users. The existing counterexamples\\n(`capabilities: [switchdev]` for Open vSwitch hardware offload and\\n`trusted=true` for Trusted Virtual Functions) are due to be cleaned up.\\nThe networking API does not define a specific format of this field.\\nThe default is an empty dictionary.\\nIf you update it with null then it is treated like {} in the response.\\nSince the port-mac-address-override extension the `device_mac_address`\\nfield of the binding:profile can be used to provide the MAC address of the\\nphysical device a direct-physical port is being bound to. If provided, then\\nthe `mac_address` field of the port resource will be updated to the MAC\\nfrom the active binding.\", \"type\": [\"null\", \"object\"]}, \"binding:vnic_type\": {\"description\": \"The type of vNIC which this port should be attached to. This is used to\\ndetermine which mechanism driver(s) to be used to bind the port.\\nThe valid values are `normal`, `macvtap`, `direct`, `baremetal`,\\n`direct-physical`, `virtio-forwarder`, `smart-nic`, `vdpa`,\\n`accelerator-direct`, `accelerator-direct-physical`\\nand `remote-managed`.\\nWhat type of vNIC is actually available depends on deployments.\\nThe default is `normal`.\", \"enum\": [\"accelerator-direct\", \"accelerator-direct-physical\", \"baremetal\", \"direct\", \"direct-physical\", \"macvtap\", \"normal\", \"remote-managed\", \"smart-nic\", \"vdpa\", \"virtio-forwarder\"], \"type\": \"string\"}, \"description\": {\"description\": \"A human-readable description for the resource.\\nDefault is an empty string.\", \"maxLength\": 255, \"type\": \"string\"}, \"device_id\": {\"description\": \"The ID of the device that uses this port.\\nFor example, a server instance or a logical router.\", \"maxLength\": 255, \"type\": \"string\"}, \"device_owner\": {\"description\": \"The entity type that uses this port.\\nFor example, `compute:nova` (server instance), `network:dhcp`\\n(DHCP agent) or `network:router_interface` (router interface).\", \"maxLength\": 255, \"type\": \"string\"}, \"device_profile\": {\"maxLength\": 255, \"type\": [\"null\", \"string\"]}, \"dns_domain\": {\"description\": \"A valid DNS domain.\", \"format\": \"hostname\", \"maxLength\": 255, \"type\": \"string\"}, \"dns_name\": {\"description\": \"A valid DNS name.\", \"format\": \"hostname\", \"maxLength\": 255, \"type\": \"string\"}, \"extra_dhcp_opts\": {\"description\": \"A set of zero or more extra DHCP option pairs. An\\noption pair consists of an option value and name.\", \"items\": {\"extraProperties\": true, \"type\": \"object\"}, \"type\": [\"array\", \"null\"], \"x-openstack\": {\"todo\": \"implementme\"}}, \"fixed_ips\": {\"description\": \"The IP addresses for the port.\\nIf you would like to assign multiple IP addresses for the port,\\nspecify multiple entries in this field.\\nEach entry consists of IP address (`ip_address`) and the subnet ID\\nfrom which the IP address is assigned (`subnet_id`).\\n\\n* If you specify both a subnet ID and an IP address, OpenStack Networking\\n  tries to allocate the IP address on that subnet to the port.\\n* If you specify only a subnet ID, OpenStack Networking allocates\\n  an available IP from that subnet to the port.\\n* If you specify only an IP address, OpenStack Networking\\n  tries to allocate the IP address if the address is a valid IP\\n  for any of the subnets on the specified network.\", \"items\": {\"properties\": {\"ip_address\": {\"description\": \"IP Address\", \"type\": \"string\"}, \"subnet_id\": {\"description\": \"The subnet ID from which the IP address is assigned\", \"type\": \"string\"}}, \"type\": \"object\"}, \"type\": \"array\"}, \"hints\": {\"description\": \"Admin-only. A dict, at the top level keyed by mechanism driver\\naliases (as defined in setup.cfg). To following values can be used to\\ncontrol Open vSwitch’s Userspace Tx packet steering feature:\\n\\n* `{\\\"openvswitch\\\": {\\\"other_config\\\": {\\\"tx-steering\\\": \\\"hash\\\"}}}`\\n* `{\\\"openvswitch\\\": {\\\"other_config\\\": {\\\"tx-steering\\\": \\\"thread\\\"}}}`\\n\\nIf omitted the default is defined by Open vSwitch.\\nThe field cannot be longer than 4095 characters.\", \"type\": [\"null\", \"object\"]}, \"mac_address\": {\"description\": \"The MAC address of the port.\\nIf unspecified, a MAC address is automatically generated.\", \"type\": \"string\"}, \"name\": {\"description\": \"Human-readable name of the resource.\\nDefault is an empty string.\", \"type\": \"string\"}, \"network_id\": {\"description\": \"The ID of the attached network.\", \"format\": \"uuid\", \"type\": \"string\"}, \"numa_affinity_policy\": {\"description\": \"The port NUMA affinity policy requested during the virtual machine\\nscheduling. Values: `None`, `required`, `preferred` or `legacy`.\", \"enum\": [null, \"legacy\", \"preferred\", \"required\", \"socket\"], \"type\": [\"null\", \"string\"]}, \"port_security_enabled\": {\"description\": \"The port security status. A valid value is\\nenabled (`true`) or disabled (`false`).\\nIf port security is enabled for the port,\\nsecurity group rules and anti-spoofing rules are applied to\\nthe traffic on the port. If disabled, no such rules are applied.\", \"type\": [\"boolean\", \"string\"]}, \"propagate_uplink_status\": {\"description\": \"The uplink status propagation of the port. Valid values are\\nenabled (`true`) and disabled (`false`).\", \"type\": [\"boolean\", \"string\"]}, \"qos_policy_id\": {\"description\": \"QoS policy associated with the port.\", \"format\": \"uuid\", \"type\": [\"null\", \"string\"]}, \"security_groups\": {\"description\": \"The IDs of security groups applied to the port.\", \"items\": {\"format\": \"uuid\", \"type\": \"string\"}, \"type\": \"array\"}, \"tags\": {\"items\": {\"type\": \"string\"}, \"type\": \"array\", \"uniqueItems\": true}, \"tenant_id\": {\"description\": \"The ID of the project that owns the resource.\\nOnly administrative and users with advsvc role can specify\\na project ID other than their own.\\nYou cannot change this value through authorization policies.\", \"maxLength\": 255, \"type\": \"string\"}}, \"type\": \"object\"}}, \"type\": \"object\"}";

...
impl<'a> RequestBuilder<'a> {
    pub fn floatingip_pool<I, K, V>(&mut self, iter: I) -> &mut Self
    where
        I: Iterator<Item = (K, V)>,
        K: Into<Cow<'a, str>>,
        V: Into<Value>,
    {
        self.floatingip_pool
...
impl RestEndpoint for Request<'_> {
    ...
    fn endpoint(&self) -> Cow<'static, str> {
        format!("floatingip-pools/{id}", id = self.id.as_ref(),).into()
    }
    ...
    fn body(&self) -> Result<Option<(&'static str, Vec<u8>)>, BodyError> {
        ...
        params.push(
            "floatingip_pool",
            serde_json::to_value(&self.floatingip_pool)?,
        );
...
pub const BODY_SCHEMA: &str = "{\"description\": \"Request of the floatingip-pools/id:put operation\", \"properties\": {\"floatingip_pool\": {\"properties\": {}, \"type\": \"object\"}}, \"type\": \"object\"}";
...
mod tests {
    ...
    fn test_service_type() {
        assert_eq!(
            Request::builder()
                .floatingip_pool(BTreeMap::<String, Value>::new().into_iter())
                .build()
                .unwrap()
                .service_type(),
            ServiceType::Network
    ...
    fn endpoint() {
        ...
        let mock = server.mock(|when, then| {
            when.method(httpmock::Method::PUT)
                .path(format!("/floatingip-pools/{id}", id = "id",));

            then.status(200)
                .header("content-type", "application/json")
                .json_body(json!({ "floatingip_pool": {} }));
        });

...
pub struct Restore<'a> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    #[builder(default, setter(into))]
    pub(crate) volume_id: Option<Option<Cow<'a, str>>>,
}
...
impl<'a> RequestBuilder<'a> {
    /// Add a single header to the Restore.
...
impl RestEndpoint for Request<'_> {
    ...
    fn endpoint(&self) -> Cow<'static, str> {
        format!("backups/{id}/restore", id = self.id.as_ref(),).into()
    }
    ...
    fn body(&self) -> Result<Option<(&'static str, Vec<u8>)>, BodyError> {
        ...
        params.push("restore", serde_json::to_value(&self.restore)?);
...
pub const BODY_SCHEMA: &str = "{\"additionalProperties\": false, \"properties\": {\"restore\": {\"additionalProperties\": false, \"properties\": {\"name\": {\"maxLength\": 255, \"minLength\": 0, \"type\": [\"null\", \"string\"]}, \"volume_id\": {\"oneOf\": [{\"type\": \"null\"}, {\"format\": \"uuid\", \"type\": \"string\"}]}}, \"type\": [\"null\", \"object\"]}}, \"required\": [\"restore\"], \"type\": \"object\"}";
...
mod tests {
    ...
    fn test_service_type() {
        assert_eq!(
            Request::builder()
                .restore(RestoreBuilder::default().build().unwrap())
                .build()
                .unwrap()
                .service_type(),
            ServiceType::BlockStorage
    ...
    fn endpoint() {
        ...
        let mock = server.mock(|when, then| {
            when.method(httpmock::Method::POST)
                .path(format!("/backups/{id}/restore", id = "id",));

            then.status(200)
                .header("content-type", "application/json")
                .json_body(json!({ "restore": {} }));
        });

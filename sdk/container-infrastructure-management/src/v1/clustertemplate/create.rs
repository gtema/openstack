...
pub const BODY_SCHEMA: &str = "{\"description\": \"API representation of a ClusterTemplate.\\n\\nThis class enforces type checking and value constraints, and converts\\nbetween the internal object model and the API representation of\\na ClusterTemplate.\", \"properties\": {\"apiserver_port\": {\"maximum\": 65535, \"minimum\": 1024, \"type\": \"integer\"}, \"cluster_distro\": {\"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"coe\": {\"description\": \"Specify the Container Orchestration Engine to use. Supported COEs\\ninclude `kubernetes`. If your environment has additional cluster drivers\\ninstalled, refer to the cluster driver documentation for the new COE names.\", \"enum\": [\"kubernetes\"], \"type\": \"string\"}, \"created_at\": {\"format\": \"date-time\", \"type\": \"string\"}, \"dns_nameserver\": {\"description\": \"The DNS nameserver for the servers and containers in the cluster to\\nuse. This is configured in the private Neutron network for the cluster.\\nThe default is `8.8.8.8`.\", \"format\": \"dnslist\", \"type\": \"string\"}, \"docker_storage_driver\": {\"description\": \"The name of a driver to manage the storage for the images and the\\ncontainer’s writable layer. The default is `devicemapper`.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"docker_volume_size\": {\"description\": \"The size in GB for the local storage on each server for the Docker daemon\\nto cache the images and host the containers. Cinder volumes provide the\\nstorage. The default is 25 GB. For the `devicemapper` storage driver,\\nthe minimum value is 3GB. For the `overlay` storage driver, the minimum\\nvalue is 1GB.\", \"minimum\": 1, \"type\": \"integer\"}, \"driver\": {\"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"external_network_id\": {\"description\": \"The name or network ID of a Neutron network to provide connectivity to the\\nexternal internet for the cluster. This network must be an external\\nnetwork, i.e. its attribute `router:external` must be `True`. The\\nservers in the cluster will be connected to a private network and\\nMagnum will create a router between this private network and the external\\nnetwork. This will allow the servers to download images, access discovery\\nservice, etc, and the containers to install packages, etc. In the opposite\\ndirection, floating IPs will be allocated from the external network to\\nprovide access from the external internet to servers and the container\\nservices hosted in the cluster.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"fixed_network\": {\"description\": \"The name or network ID of a Neutron network to provide connectivity to\\nthe internal network for the cluster.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"fixed_subnet\": {\"description\": \"Fixed subnet that are using to allocate network address for nodes in\\ncluster.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"flavor_id\": {\"description\": \"The nova flavor ID or name for booting the node servers. The default is\\n`m1.small`.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"floating_ip_enabled\": {\"description\": \"Whether enable or not using the floating IP of cloud provider. Some\\ncloud providers used floating IP, some used public IP, thus Magnum\\nprovide this option for specifying the choice of using floating IP.\", \"format\": \"boolean\", \"type\": \"string\"}, \"hidden\": {\"description\": \"Indicates whether the ClusterTemplate is hidden or not, the default\\nvalue is false.\", \"format\": \"boolean\", \"type\": \"string\"}, \"http_proxy\": {\"description\": \"The IP address for a proxy to use when direct http access from the servers\\nto sites on the external internet is blocked. This may happen in certain\\ncountries or enterprises, and the proxy allows the servers and\\ncontainers to access these sites. The format is a URL including a port\\nnumber. The default is `None`.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"https_proxy\": {\"description\": \"The IP address for a proxy to use when direct https access from the\\nservers to sites on the external internet is blocked. This may happen in\\ncertain countries or enterprises, and the proxy allows the servers and\\ncontainers to access these sites. The format is a URL including a port\\nnumber. The default is `None`.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"image_id\": {\"description\": \"The name or UUID of the base image in Glance to boot the servers for the\\ncluster. The image must have the attribute `os_distro` defined as\\nappropriate for the cluster driver.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"insecure_registry\": {\"description\": \"The URL pointing to users’s own private insecure docker registry to\\ndeploy and run docker containers.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"keypair_id\": {\"description\": \"The name of the SSH keypair to configure in the cluster servers\\nfor ssh access. Users will need the key to be able to ssh to the servers in\\nthe cluster. The login name is specific to the cluster driver, for\\nexample with fedora-atomic image, default login name is `fedora`.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"labels\": {\"additionalProperties\": {\"type\": [\"boolean\", \"integer\", \"number\", \"string\"]}, \"description\": \"Arbitrary labels in the form of `key=value` pairs. The accepted keys and\\nvalid values are defined in the cluster drivers. They are used as a way\\nto pass additional parameters that are specific to a cluster driver.\", \"type\": \"object\"}, \"links\": {\"items\": {\"description\": \"A link representation.\", \"properties\": {\"created_at\": {\"format\": \"date-time\", \"type\": \"string\"}, \"href\": {\"type\": \"string\"}, \"rel\": {\"type\": \"string\"}, \"type\": {\"type\": \"string\"}, \"updated_at\": {\"format\": \"date-time\", \"type\": \"string\"}}, \"type\": \"object\"}, \"type\": \"array\"}, \"master_flavor_id\": {\"description\": \"The flavor of the master node for this cluster template.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"master_lb_enabled\": {\"description\": \"Since multiple masters may exist in a cluster, a Neutron load balancer\\nis created to provide the API endpoint for the cluster and to direct\\nrequests to the masters. In some cases, such as when the LBaaS service is\\nnot available, this option can be set to `false` to create a cluster\\nwithout the load balancer. In this case, one of the masters will serve as\\nthe API endpoint. The default is `true`, i.e. to create the load\\nbalancer for the cluster.\", \"format\": \"boolean\", \"type\": \"string\"}, \"name\": {\"description\": \"Name of the resource.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"network_driver\": {\"description\": \"The name of a network driver for providing the networks for the containers.\\nNote that this is different and separate from the Neutron network for the\\ncluster. The operation and networking model are specific to the\\nparticular driver.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"no_proxy\": {\"description\": \"When a proxy server is used, some sites should not go through the proxy\\nand should be accessed normally. In this case, users can specify these\\nsites as a comma separated list of IPs. The default is `None`.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"project_id\": {\"type\": \"string\"}, \"public\": {\"description\": \"Access to a cluster template is normally limited to the admin,\\nowner or users within the same tenant as the owners. Setting this flag\\nmakes the cluster template public and accessible by other users.\\nThe default is not public.\", \"format\": \"boolean\", \"type\": \"string\"}, \"registry_enabled\": {\"description\": \"Docker images by default are pulled from the public Docker registry,\\nbut in some cases, users may want to use a private registry. This option\\nprovides an alternative registry based on the Registry V2: Magnum will\\ncreate a local registry in the cluster backed by swift to host the\\nimages. The default is to use the public registry.\", \"format\": \"boolean\", \"type\": \"string\"}, \"server_type\": {\"description\": \"The servers in the cluster can be `vm` or `baremetal`. This\\nparameter selects the type of server to create for the cluster.\\nThe default is `vm`.\", \"enum\": [\"bm\", \"vm\"], \"type\": \"string\"}, \"tags\": {\"description\": \"Administrator tags for the cluster template.\", \"maxLength\": 255, \"type\": \"string\"}, \"tls_disabled\": {\"description\": \"Transport Layer Security (TLS) is normally enabled to secure the\\ncluster. In some cases, users may want to disable TLS in the\\ncluster, for instance during development or to troubleshoot certain\\nproblems. Specifying this parameter will disable TLS so that users can\\naccess the COE endpoints without a certificate. The default is TLS enabled.\", \"format\": \"boolean\", \"type\": \"string\"}, \"updated_at\": {\"format\": \"date-time\", \"type\": \"string\"}, \"user_id\": {\"type\": \"string\"}, \"uuid\": {\"format\": \"uuid\", \"type\": \"string\"}, \"volume_driver\": {\"description\": \"The name of a volume driver for managing the persistent storage for the containers. The functionality supported are specific to the driver.\", \"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}}, \"required\": [\"image_id\"], \"type\": \"object\"}";

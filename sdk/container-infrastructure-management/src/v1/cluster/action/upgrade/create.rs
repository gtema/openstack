...
pub struct Request<'a> {
    ...
    pub(crate) cluster_template: Option<Cow<'a, str>>,
...
impl<'a> RequestBuilder<'a> {
    /// Add a single header to the Upgrade.
...
impl RestEndpoint for Request<'_> {
    ...
    fn endpoint(&self) -> Cow<'static, str> {
        "clusters/actions/upgrade".to_string().into()
    }
    ...
    fn body(&self) -> Result<Option<(&'static str, Vec<u8>)>, BodyError> {
        ...
        if let Some(val) = &self.cluster_template {
            params.push("cluster_template", serde_json::to_value(val)?);
        }
        if let Some(val) = &self.created_at {
            params.push("created_at", serde_json::to_value(val)?);
        }
        if let Some(val) = &self.max_batch_size {
            params.push("max_batch_size", serde_json::to_value(val)?);
        }
...
pub const BODY_SCHEMA: &str = "{\"description\": \"API object for handling upgrade requests.\\n\\nThis class enforces type checking and value constraints.\", \"properties\": {\"cluster_template\": {\"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"created_at\": {\"format\": \"date-time\", \"type\": \"string\"}, \"max_batch_size\": {\"minimum\": 1, \"type\": \"integer\"}, \"nodegroup\": {\"maxLength\": 255, \"minLength\": 1, \"type\": \"string\"}, \"updated_at\": {\"format\": \"date-time\", \"type\": \"string\"}}, \"type\": \"object\"}";
    ...
    fn endpoint() {
        ...
        let mock = server.mock(|when, then| {
            when.method(httpmock::Method::POST)
                .path("/clusters/actions/upgrade".to_string());

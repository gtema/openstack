...
//!
//! ```rust
//! let cfg = openstack_sdk_core::config::ConfigFile::new().unwrap();
//! let profile = cfg
...
//!
//! ```no_run
//! let cfg = openstack_sdk_core::config::ConfigFile::builder()
//!     .add_source("c1.yaml")
...
//!
//! ```no_run
//! let cfg = openstack_sdk_core::config::ConfigFile::new_with_user_specified_configs(
//!     Some("c1.yaml"),
//!     Some("s2.yaml"),
    ...
    S: Serializer,
{
    if let Some(val) = secret {
        serializer.serialize_str(val.expose_secret())
    } else {
...
impl ConfigFile {
    ...
    pub fn get_cloud_config<S: AsRef<str>>(
        ...
        if let Some(clouds) = &self.clouds
            && let Some(cfg) = clouds.get(cloud_name.as_ref())
        {

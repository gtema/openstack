...
//! `/v3/projects` REST operations bindings of Identity
...
pub enum IdentityProjectApiRequest {
    /// Delete
    Delete(Box<IdentityProjectDelete>),
    /// List
    List(Box<IdentityProjectList>),
}
...
impl ConfirmableRequest for IdentityProjectApiRequest {
    fn get_confirm_message(&self) -> Option<String> {
        match &self {
            IdentityProjectApiRequest::Delete(req) => req.get_confirm_message(),
...
impl ExecuteApiRequest for IdentityProjectApiRequest {
    async fn execute_request(
        ...
        match self {
            IdentityProjectApiRequest::Delete(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
            IdentityProjectApiRequest::List(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
        }
        Ok(())

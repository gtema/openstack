...
//! `/v2.0/networks/{id}` REST operations bindings of Network
...
pub enum NetworkNetworkApiRequest {
    /// Delete
    Delete(Box<NetworkNetworkDelete>),
    /// List
    List(Box<NetworkNetworkList>),
}
...
impl ExecuteApiRequest for NetworkNetworkApiRequest {
    async fn execute_request(
        ...
        match self {
            NetworkNetworkApiRequest::Delete(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
            NetworkNetworkApiRequest::List(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
        }
        Ok(())

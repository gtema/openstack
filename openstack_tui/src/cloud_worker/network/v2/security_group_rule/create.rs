    ...
    for openstack_sdk::api::network::v2::security_group_rule::create::SecurityGroupRuleBuilder<'_>
{
    ...
    fn try_from(value: &SecurityGroupRule) -> Result<Self, Self::Error> {
        ...
        if let Some(val) = &value.port_range_min {
            ep_builder.port_range_min(val.clone().map(Into::into));
        }
        if let Some(val) = &value.port_range_max {
            ep_builder.port_range_max(val.clone().map(Into::into));
        }
...
mod tests {
    ...
    #[test]
    fn test_convert() {
        let sot: NetworkSecurityGroupRuleCreate =
            serde_json::from_value(json!({ "security_group_rule": {
              "direction": "egress",
              "ethertype": "IPv4",
              "port_range_max": 8888,
              "port_range_min": 8888,
              "security_group_id": "dda22912-94d2-4db8-8d3b-294415ba6fbb",
            }}))
            .unwrap();

        let sdk = RequestBuilder::try_from(&sot).unwrap();
    }
}

...
//! `/v2.0/subnets/{id}` REST operations bindings of Network
...
pub enum NetworkSubnetApiRequest {
    /// Delete
    Delete(Box<NetworkSubnetDelete>),
    /// List
    List(Box<NetworkSubnetList>),
}
...
impl ExecuteApiRequest for NetworkSubnetApiRequest {
    async fn execute_request(
        ...
        match self {
            NetworkSubnetApiRequest::Delete(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
            NetworkSubnetApiRequest::List(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
        }
        Ok(())

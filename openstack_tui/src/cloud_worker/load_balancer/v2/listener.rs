...
//! `/v2/lbaas/listeners/{id}` REST operations bindings of LoadBalancer
...
pub enum LoadBalancerListenerApiRequest {
    /// Delete
    Delete(Box<LoadBalancerListenerDelete>),
    /// Get
    Get(Box<LoadBalancerListenerShow>),
    /// List
    List(Box<LoadBalancerListenerList>),
}
...
impl ConfirmableRequest for LoadBalancerListenerApiRequest {
    fn get_confirm_message(&self) -> Option<String> {
        match &self {
            LoadBalancerListenerApiRequest::Delete(req) => req.get_confirm_message(),

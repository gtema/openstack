...
//! `/v2/lbaas/pools/{pool_id}/members/{id}` REST operations bindings of LoadBalancer
...
pub enum LoadBalancerPoolMemberApiRequest {
    /// Delete
    Delete(Box<LoadBalancerPoolMemberDelete>),
    /// Get
    Get(Box<LoadBalancerPoolMemberShow>),
    /// List
    List(Box<LoadBalancerPoolMemberList>),
}
...
impl ExecuteApiRequest for LoadBalancerPoolMemberApiRequest {
    async fn execute_request(
        ...
        match self {
            LoadBalancerPoolMemberApiRequest::Delete(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
            LoadBalancerPoolMemberApiRequest::Get(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
            LoadBalancerPoolMemberApiRequest::List(req) => {
                req.execute_request(session, request, app_tx).await?;
            }
        }
        Ok(())
